...
const { rolloverIfNeeded, computeMatchResult, occupiedParticipants, MATCH_INCLUDE } = require("./weeks");
...
async function sweep() {
  try {
    ...
    for (const match of week.matches) {
      ...
      // catch-all: if a match somehow has everyone's submission and no
      // verdict yet (e.g. the server restarted between the last submit and
      // the judge call), run it now.
      if (occupied >= 2 && match.submissions.length === occupied && !match.verdict) {
        await runAiJudge(match.id);
      }
      // Same lock trigger as isMatchRevealed() (weeks.js): everyone
      // submitted, OR the deadline passed regardless of how many did — a
      // partial match still needs to lock so it eventually shows up in the
      // archive and gets finalized (as a no-contest — computeMatchResult
      // stays pending with fewer than 2 submissions, so it never counts
      // for/against anyone) instead of sitting open forever.
      if (!match.lockedAt && ((occupied >= 2 && match.submissions.length === occupied) || new Date() >= new Date(week.deadline))) {
...
/** Fires once per match, ~24h before the week's deadline, to whoever
 *  hasn't submitted yet. Skipped entirely if everyone already has. */
async function maybeSendReminder(match, week) {
  ...
  const slotUserIds = occupiedParticipants(match).map((p) => p.userId);
  ...
  if (!missing.length) return; // everyone already in — nothing to remind anyone about
 ...
 *  deriveStandings in weeks.js), and let everyone in it know. */
